...
public enum BossTeleport {
    ...
    CHAOS_ELEMENTAL(BossTask.BossTasks.CHAOS_ELEMENTAL,
            new Teleport("Chaos Elemental", Tile.of(3279, 3925, 0), player -> player.getControllerManager().startController(new WildernessController()))),
    GENERAL_GRAARDOR(BossTask.BossTasks.GENERAL_GRAARDOR,
            new Teleport("General Graardor", Tile.of(2858, 5354, 2), player -> player.getControllerManager().startController(new GodwarsController()))),
    COMMANDER_ZILYANA(BossTask.BossTasks.COMMANDER_ZILYANA,
            new Teleport("Commander Zilyana", Tile.of(2912, 5267, 0), player -> player.getControllerManager().startController(new GodwarsController()))),
    KRIL_TSUTAROTH(BossTask.BossTasks.KRIL_TSUTAROTH,
            new Teleport("K'ril Tsutaroth", Tile.of(2926, 5340, 2), player -> player.getControllerManager().startController(new GodwarsController()))),
    KREE_ARRA(BossTask.BossTasks.KREE_ARRA,
            new Teleport("Kree'arra", Tile.of(2836, 5291, 2), player -> player.getControllerManager().startController(new GodwarsController()))),
    NEX(BossTask.BossTasks.NEX,
            new Teleport("Nex", Tile.of(2885, 5284, 2), player -> player.getControllerManager().startController(new GodwarsController()))),
    KING_BLACK_DRAGON(BossTask.BossTasks.KING_BLACK_DRAGON,
            new Teleport("King Black Dragon", Tile.of(2273, 4683, 0))),
    KALPHITE_QUEEN(BossTask.BossTasks.KALPHITE_QUEEN,
            new Teleport("Kalphite Queen", Tile.of(3512, 9494, 2))),
    CORPOREAL_BEAST(BossTask.BossTasks.CORPOREAL_BEAST,
            new Teleport("Corporeal Beast", Tile.of(2965, 4383, 2))),
    DAGANNOTH_REX(BossTask.BossTasks.DAGANNOTH_REX,
            new Teleport("Dagannoth Rex", Tile.of(1923, 4366, 0))),
    DAGANNOTH_PRIME(BossTask.BossTasks.DAGANNOTH_PRIME,
            new Teleport("Dagannoth Prime", Tile.of(1923, 4366, 0))),
    DAGANNOTH_SUPREME(BossTask.BossTasks.DAGANNOTH_SUPREME,
            new Teleport("Dagannoth Supreme", Tile.of(1923, 4366, 0))),
    TORMENTED_DEMON(BossTask.BossTasks.TORMENTED_DEMON,
            new Teleport("Tormented Demon", Tile.of(2569, 5734, 0))),
    ;
